...
class Independent_noisy_layer(nn.Module):
    def __init__(self, input_p, output_q):
        super(Independent_noisy_layer, self).__init__()
...
class Q(nn.Module):
    def __init__(self, state_space, action_space, atom_size, vmin, vmax):
        ...
        self.fc_v = Factorised_noisy_layer(32, atom_size) ## rainbow 논문에서 명시되어 있음.
        self.fc_a = Factorised_noisy_layer(32, action_space * atom_size) ## rainbow 논문에서 명시되어 있음.
    ...
    def forward(self, x):
        ...
        V_values = self.fc_v(x).view(-1, 1, self.atom_size)
        Advantages = self.fc_a(x).view(-1, self.action_space, self.atom_size)

        atoms = V_values + (Advantages - Advantages.mean(dim=1, keepdim=True))
